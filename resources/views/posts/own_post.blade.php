...
<script>
    ...
    const button = $(this);
    setTimeout(function() {
      // Zrušit :focus stav na tlačítku po uplynutí 1 sekundy
      button.blur();
    }, 500);
  });
...
$('#body').on('keydown', function (event) {
        ...
        if (event.key === 'Enter') {
            event.preventDefault(); // Zabraň výchozímu chování, jako je vytváření nového řádku
...
$('#add-comment-form').on('submit', function (e) {
    ...
    success: function (response) {
        ...
        // Vytvořte HTML pro nový komentář
        let newCommentHtml = `
            <li>
                <div class="header">
